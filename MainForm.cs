...
ImageForm IF;
        ...
        {
            myConnectionString = "server=localhost;uid=root;" +"pwd=null;database=parking";
            try
            {
                conn = new MySql.Data.MySqlClient.MySqlConnection();
                conn.ConnectionString = myConnectionString;
                //connection = new MySqlConnection(conn_string.ToString());
                MySqlDataAdapter adapter = new MySqlDataAdapter("SELECT * FROM vehicle_details", connection);
                conn.Open();
            }
                    ...
                    using (Image<Gray, byte> grayframe = new Image<Gray, byte>(src))
                    {
                       ...
                       grayframe.DetectHaarCascade(haar, 1.1, 8, HAAR_DETECTION_TYPE.DO_CANNY_PRUNING, new Size(0, 0))[0];
                        ...
                        if (isface)
                        {
                            ...
                            PlateImagesList[0] = PlateImagesList[0].Resize(400, 400, Emgu.CV.CvEnum.INTER.CV_INTER_LINEAR);
                            //PlateImagesList[0] = PlateImagesList[0].Resize(800, 600, Emgu.CV.CvEnum.INTER.CV_INTER_LINEAR);
                            return;
                        }
        ...
        private void Reconize(string link, out Image hinhbienso, out string bienso, out string bienso_text)
        {
            ...
            if (PlateImagesList.Count != 0)
            {
                ...
                Bitmap color;
                int c = con.IdentifyContours(src.ToBitmap(), 50, true, out grayframe, out color, out listRect);  // find contour
                //int z = con.count;
        ...
        private void pictureBox_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                List<string> ls = Devices.get_all_cam();
                for(int i = 0; i<=2 & i < ls.Count; i++)
                {
                    m.MenuItems.Add(ls[i], (s, e2) => {
                        ...
                        cam[menuItem.Index] = new Devices();
